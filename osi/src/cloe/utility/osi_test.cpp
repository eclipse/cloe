...
#include "cloe/utility/osi_message_handler.hpp"  // for transform_ego_coord_from_osi_data, ...
#include "cloe/utility/osi_utils.hpp"            // for pose_to_osi_position_orientation, ...
...
TEST(osi, eigen_pose) {
  ...
  Eigen::Isometry3d pose = cloe::utility::osi_position_orientation_to_pose(base);
  // Inverse conversion.
  osi3::BaseMoving base_out;
  cloe::utility::pose_to_osi_position_orientation(pose, base_out);
...
TEST(osi, transf_base_mov) {
  ...
  cloe::utility::osi_transform_base_moving(ego_base, obj_base);
...
TEST(osi, transform_ego_coord) {
  ...
  cloe::utility::transform_ego_coord_from_osi_data(obj_dims, ego);
...
TEST(osi, transform_obj_coord) {
  ...
  cloe::utility::transform_obj_coord_from_osi_data(sensor_pose, obj_dims, obj);
...
TEST(osi, vehicle_classification) {
  ...
  cloe::utility::from_osi_mov_obj_type_classification(osi_obj, obj.classification);
