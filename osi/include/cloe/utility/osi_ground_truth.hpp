...
class OsiGroundTruth {
 public:
  ...
  OsiGroundTruth(const OsiGroundTruth&) = default;
  OsiGroundTruth(OsiGroundTruth&&) = delete;
  OsiGroundTruth& operator=(const OsiGroundTruth&) = default;
  OsiGroundTruth& operator=(OsiGroundTruth&&) = delete;
  ...
  [[nodiscard]] const osi3::GroundTruth& get_gt() const {
    if (gt_ptr_ == nullptr) {
      error();
    }
    return *gt_ptr_;
   ...
   */
  [[nodiscard]] const Eigen::Vector3d& get_veh_coord_sys_info(int obj_id) const {
   ...
   */
  [[nodiscard]] const Eigen::Vector3d& get_mov_obj_dimensions(int obj_id) const {
   ...
   */
  [[nodiscard]] uint64_t get_ego_id() const {
   ...
   */
  [[nodiscard]] const osi3::MovingObject* get_moving_object(uint64_t id) const;
