...
class OsiTransceiver {
 public:
   ...
   * that is not empty.
   */
  [[nodiscard]] virtual bool has_sensor_data() const = 0;
   ...
   * that is not empty.
   */
  [[nodiscard]] virtual bool has_sensor_view() const = 0;
   ...
   * that is not empty.
   */
  [[nodiscard]] virtual bool has_ground_truth() const = 0;
  ...
  friend void to_json(fable::Json& j, const OsiTransceiver& t) { t.to_json(j); }
};
