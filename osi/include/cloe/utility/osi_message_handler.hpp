...
#include <cloe/utility/osi_utils.hpp>
...
void from_osi_detected_moving_object_alt(const osi3::DetectedMovingObject& osi_mo,
                                         const OsiGroundTruth& ground_truth, Object& obj);
 ...
 */
struct SensorMockConf : public fable::Confable {
  using Target = SensorMockTarget;
  using Level = SensorMockLevel;

  SensorMockConf() = default;
  ~SensorMockConf() noexcept override = default;
  ...
  CONFABLE_SCHEMA(SensorMockConf) {
    return fable::Schema{
        // clang-format off
        {"mounting_position", fable::Schema(&level[Target::MountingPosition], "mock level for sensor mounting position")},
        {"detected_moving_objects", fable::Schema(&level[Target::DetectedMovingObject], "mock level for detected moving objects")},
        {"detected_static_objects", fable::Schema(&level[Target::DetectedStaticObject], "mock level for detected stationary objects")},
        {"detected_lane_boundaries", fable::Schema(&level[Target::DetectedLaneBoundary], "mock level for detected lane boundaries")},
        // clang-format on
    };
  }
...
class OsiMsgHandler {
 public:
   ...
   */
  template <typename T>
  void process_osi_msgs(const Sync& s, const bool& restart, Duration& osi_time);
   ...
   */
  virtual void process_received_msg(osi3::SensorData* osi_sd, Duration& osi_time);
   ...
   * \param osi_time Timestamp of the OSI message.
   */
  virtual void process_received_msg(osi3::SensorView* osi_sv, Duration& osi_time);
   ...
   * \param osi_time Timestamp of the OSI message.
   */
  virtual void process_received_msg(osi3::GroundTruth* osi_gt, Duration& osi_time);
   ...
   */
  virtual void convert_to_cloe_data(bool has_eh,
  ...
  void from_osi_boundary_points(const osi3::LaneBoundary& osi_lb, LaneBoundary& lb,
   ...
   */
  virtual void store_ego_object(std::shared_ptr<Object> ego_obj) = 0;
   ...
   */
  virtual void store_object(std::shared_ptr<Object> obj) = 0;
   ...
   */
  virtual void store_lane_boundary(const LaneBoundary& lb) = 0;
  ...
  */
  [[nodiscard]] Duration osi_timestamp_to_simtime(const osi3::Timestamp& timestamp) const;
  ...
  */
  ...
  [[nodiscard]] SensorMockLevel get_mock_level(SensorMockTarget trg_type) const {
  ...
  */
  ...
  friend void to_json(fable::Json& j, const OsiMsgHandler& c) {
    j = fable::Json{
 ...
 protected:
  ...
  Duration init_time_ = Duration(-1);
