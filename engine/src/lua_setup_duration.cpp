...
#include <fable/utility/chrono.hpp> // for parse_duration

namespace cloe {

void register_usertype_duration(sol::table& lua) {
  Duration (*parse_duration_ptr)(const std::string&) = ::fable::parse_duration<cloe::Duration>;
  std::string (*to_string_ptr)(const Duration&) = ::fable::to_string<cloe::Duration>;
