...
#pragma warning(pop)
        ...
        const char* type = (tt == LUA_TSTRING) ? lua_tostring(L, -1) : luaL_typename(L, index);
...
class debug_info {
 public:
  ...
  debug_info(const debug_info& other)
      : state_(other.state_), debug_(other.debug_), got_debug_(other.got_debug_) {}
  ...
  std::vector<json::value> eval(const char* script, bool global = true, bool upvalue = true,
    ...
    std::string error;
    std::vector<json::value> ret = eval(script, error, global, upvalue, local, object_depth);
    if (!error.empty()) {
  ...
  local_vars_type get_local_vars(int object_depth = 0) {
    ...
    if (is_variadic_arg()) {
      ...
      localvars.push_back(std::pair<std::string, json::value>("(*vararg)", json::value(va)));
    }
#endif
    return localvars;
  }
  ...
  local_vars_type get_upvalues(int object_depth = 0) {
    ...
    while (const char* varname = lua_getupvalue(state_, -1, upvno++)) {
      localvars.push_back(
          std::pair<std::string, json::value>(varname, utility::to_json(state_, -1, object_depth)));
      lua_pop(state_, 1);
    }
...
class debugger {
 public:
  ...
  debugger(lua_State* L) : state_(0), pause_(true), step_type_(STEP_ENTRY) { reset(L); }
  ~debugger() { reset(); }
  ...
  void clear_breakpoints(const std::string& file, int line = -1) {
    line_breakpoints_.erase(std::remove_if(line_breakpoints_.begin(), line_breakpoints_.end(),
  ...
  const line_breakpoint_type& line_breakpoints() const { return line_breakpoints_; }
  ...
  void set_pause_handler(pause_handler_type handler) { pause_handler_ = handler; }
 ...
 private:
  ...
  bool breakpoint_hit_cond(const breakpoint_info& breakpoint, debug_info& debuginfo) {
    if (!breakpoint.hit_condition.empty()) {
      json::array condret =
          debuginfo.eval((std::to_string(breakpoint.hit_count) + breakpoint.hit_condition).c_str());
