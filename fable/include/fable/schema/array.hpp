 ...
 private:
  ...
  void deserialize_from_object(Type& array, const Conf& c) const {
    for (const auto& kv : c->items()) {
      ...
      // - We can't exactly use from_json, because that would bypass the prototype.
      // - We can't use prototype_.deserialize, because that creates a new
      //   type and replaces any previously existing data.
      // - So we need to add `deserialize_to` and `serialize_from` to all
