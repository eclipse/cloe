...
class ESMiniEnvData {
 public:
  ESMiniEnvData() = delete;
  virtual ~ESMiniEnvData() = default;

  /**
   * Construct a new instance of ESMiniEnvData with the given name.
   */
  explicit ESMiniEnvData(const std::string& name, double filter_dist) : name_(name) {
    // Set object sensor into the ego vehicle origin.
    mount_.setIdentity();
    // Only set frustum far range and keep all other parameters as default (=surrround view).
    frustum_.clip_far = filter_dist;
  }
   ...
   */
  virtual cloe::Duration time() const { return env_data_time_; }
  ...
  friend void to_json(cloe::Json& j, const ESMiniEnvData& s) {
    j = cloe::Json{
        {"env_data_time", s.env_data_time_},           {"restart", s.restart_},
