...
class ESMiniSimpleEgoModel {
 public:
  ...
  [[nodiscard]] double get_throttle_from_acceleration(double trg_accel) const {
...
class ESMiniEgoControl : public cloe::LatLongActuator {
 public:
  ESMiniEgoControl(uint64_t id) : LatLongActuator("esmini/lat_long_actuator"), ego_id_(id) {
   ...
   ~ESMiniEgoControl() override = default;
  ...
  void step(const cloe::Sync& s) {
    double trg_throttle = 0;
    double trg_steering_angle = 0;
    ...
    SE_ReportObjectPosXYH(0, 0.0F, ego_state.x, ego_state.y, ego_state.h);
    SE_ReportObjectSpeed(0, ego_state.speed);
