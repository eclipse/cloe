...
class ESMiniSimulator : public cloe::Simulator {
 public:
  ...
  ESMiniSimulator& operator=(const ESMiniSimulator&) = default;
  ESMiniSimulator& operator=(ESMiniSimulator&&) = delete;
  ESMiniSimulator(const std::string& name, ESMiniConfiguration c)
      : Simulator(name), config_(std::move(c)) {}

  ~ESMiniSimulator() noexcept override = default;
  ...
  void configure_ego_vehicles() {
    ...
    for (int i = 0; i < SE_GetNumberOfObjects(); ++i) {
      auto id = SE_GetId(i);
      const auto* name = SE_GetObjectName(id);
      if (name != nullptr) {
  ...
  cloe::Duration process(const cloe::Sync& sync) final {
    ...
    auto step_size_sec = std::chrono::duration<float>(sync.step_width()).count();
...
EXPORT_CLOE_PLUGIN(cloe_esmini::ESMiniFactory)
