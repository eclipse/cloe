...
struct DemoPrinterConf : public cloe::Confable {
  CONFABLE_FRIENDS(DemoPrinterConf)
};

class DemoPrinter : public cloe::Controller {
 public:
  ...
  cloe::Duration process(const cloe::Sync& sync) override {
    ...
    log->info("Step {} @ {}", sync.step(), sync.time().count());
...
DEFINE_CONTROLLER_FACTORY(DemoPrinterFactory, DemoPrinterConf, "demo_printer",
                          "print a lot of information")

std::unique_ptr<cloe::Controller> DemoPrinterFactory::make(const cloe::Conf&) const {
  return std::make_unique<DemoPrinter>(this->name());
}

}  // namespace demo

// Register factory as plugin entrypoint
EXPORT_CLOE_PLUGIN(demo::DemoPrinterFactory)
