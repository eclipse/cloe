...
class CloeModels(ConanFile):
    ...
    def package_info(self):
        ...
        self.cpp_info.set_property("cmake_file_name", "cloe-data-broker-bindings")
        self.cpp_info.set_property("cmake_target_name", "cloe::data-broker-bindings")
        self.cpp_info.set_property("pkg_config_name", "cloe-data-broker-bindings")

        # Make sure we can find the library, both in editable mode and in the
        # normal package mode:
        if not self.in_local_cache:
            self.cpp_info.libs = ["cloe-data-broker-bindings"]
        else:
